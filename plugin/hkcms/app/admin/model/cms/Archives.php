...
class Archives extends Model
{
    ...
    {
        $fields = Db::name('model_field')->where(['status'=>'normal','model_id'=>$this->model->id])->cache(app()->isDebug()?false:3600)->select()->toArray();
        ...
        if (isset($obj['content'])) {
            ...
            // *内容标签内链*
            $tags = (new Tags)->where(['autolink'=>1])->cache(app()->isDebug()?false:3600)->select();
            $box = [];
            // 取出所有标签防止被替换
    ...
    public function tagContent($tag, $catData, $modelData, &$page)
    {
        ...
        if ($tag['aid']) { // 指定单条文档
            ...
            $array = $obj[0];
            $modelInfo = \app\admin\model\cms\Model::where(['status'=>'normal','id'=>$array['model']['id']])->cache(app()->isDebug()?false:3600)->find();
            if (empty($modelInfo)) {
                return [];
            }
            ...
            // 获取扩展字段
            $fields = \think\facade\Db::name('model_field')->where(['status'=>'normal','model_id'=>$array['model']['id']])->cache(app()->isDebug()?false:3600)->select()->toArray();
            $array = [$array];
...
//            if (empty($modelData)) {
//                // 未指定模型使用栏目来查数据
//                if (empty($tag['model']) && !empty($catData)) {
//                    $tag['model'] = isset($catData[0]['model_id'])?$catData[0]['model_id']:$catData['model_id'];
//                } else if (empty($tag['model']) && $tag['catid']) {
//                    $catid = explode(',',(string)$tag['catid']);
//                    $cateInfo = Category::where(['id'=>$catid[0],'status'=>'normal','lang'=>$tag['lang']])->cache(env('APP_DEBUG')?false:3600)->find();
...
//
//                }
//                $modelData = \app\admin\model\cms\Model::where(['status'=>'normal','id'=>$tag['model']])->cache()->find();
//                if (empty($modelData)) {
//                    return [];
//                }
//            }
            ...
            if ($sort!='_default' && Validate::is($sort, 'alphaDash')) {
                $mf = ModelField::where(['field_name'=>$sort])->cache(app()->isDebug()?false:3600)->find();
                if (!empty($mf)) {
            ...
            $filterField = ModelField::where(['model_id'=>$tag['model'],'is_filter'=>1])->cache(app()->isDebug()?false:3600)->select()->toArray();
...
//            if ($modelData['controller']=='Single' && (!isset($tag['inlist']) || $tag['inlist']==0)) {
//                $tag['insub'] = 0;
//            }
            ...
            // 获取扩展字段
            $fields = \think\facade\Db::name('model_field')->where(['status'=>'normal','model_id'=>$modelData->id])->cache(app()->isDebug()?false:3600)->select()->toArray();
    ...
    public function tagArcone($tag, $model)
    {
        ...
        $array = [$info->toArray()];
        // 获取扩展字段
        $fields = \think\facade\Db::name('model_field')->where(['status'=>'normal','model_id'=>$model->id])->cache(app()->isDebug()?false:3600)->select()->toArray();
        // 字段格式化
        foreach ($array as $key=>$value) {
            foreach ($fields as $k=>$v) {
                field_format($v, $array[$key]);
