...
class ConfigService extends BaseService
{
    ...
    public function site(string $name = '', string $module = '')
    {
        ...
        // 获取语言
        $lang = app()->make(LangService::class)->getLang($module);
        $value = $name ? config($module.$lang.'_site.'.$name) : config($module.$lang.'_site');
        if (empty($value)) { // 为空的情况获取缓存
            $site = Cache::get($module.$lang.'_site');
            if (empty($site)) {
                ...
                config($site,$module.$lang.'_site');
                Cache::tag(self::CACHE_TAG[0])->set($module.$lang.'_site', $site);
            }
            $value = $name ? ($site[$name] ?? '') : $site;
        }
        // 上传地址不写入缓存
