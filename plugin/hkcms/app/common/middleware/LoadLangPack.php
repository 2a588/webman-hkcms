...
class LoadLangPack
{
    ...
    public function detect(Request $request): string
    {
        ...
        if ($site['content_lang_on']==1 && $name=='index') {
            ...
            // 判断标识是否在当前前台语言包中。
            $contentLang = app()->make(LangService::class)->getListByModule('content');
            $contentLangArr = array_column($contentLang->toArray(), 'mark');
            if (!empty($lang) && isset($contentLangArr[$lang[0]])) {
                return $lang[0];
            }
        }
        ...
        // 允许的语言包
        $langList = [];
        $contentLang = app()->make(LangService::class)->getListByModule($name);
        $contentLangArr = array_column($contentLang->toArray(), 'mark');
        if (!empty($contentLangArr)) {
            $langList = $contentLangArr;
        }
        if (empty($langSet)) {
