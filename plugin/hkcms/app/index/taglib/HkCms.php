...
class HkCms extends TagLib
{
    ...
    protected $tags   =  [
        ...
        // 内容列表标签
        'content' => ['attr'=>'catid,field,model,order,num,where,page,id,empty,mod,key,cache,tagid,flag,inlist,aid,aids', 'close'=>1],
        // 生成列表分页页码标签
    ...
    public function tagLanguage($tag, $content)
    {
        ...
        $parseStr = '<?php'."\r\n";
        $parseStr .= '$__language__ = site("index_lang_on")==1?app()->make(\app\common\services\lang\LangService::class)->getSearchList(["status"=>1,"module"=>1])->toArray():[];'."\r\n";
        $parseStr .= ''."\r\n";
        $parseStr .= '?>';
